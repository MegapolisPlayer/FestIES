			...
			const i = setInterval(() => {
				if (this.id <= this.buildings.length) {
					...
					for (let i = 0; i < this.id; i++) {
						context?.drawImage(this.on[this.buildings[returning ? this.buildings.length - 1 - i : i].type] as HTMLImageElement, this.buildings[returning ? this.buildings.length - 1 - i : i].x, context.canvas.height - JB_BUILDING_HEIGHT);
					}

					for (let i = this.id; i < this.buildings.length; i++) {
						context?.drawImage(this.off[this.buildings[returning ? this.buildings.length - 1 - i : i].type] as HTMLImageElement, this.buildings[returning ? this.buildings.length - 1 - i : i].x, context.canvas.height - JB_BUILDING_HEIGHT);
					}
				}
